...
- Replaced architecture references with `Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates)` and `1. Database Layer (PostgreSQL schema, migrations)`
- Replaced component/service examples with `{{PROJECT_EXAMPLES.*}}` variables
- Replaced project name "MeatyPrompts" with `MeatyGifts`
...
- `MeatyGifts` - Project name throughout documentation
...
- `Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates)` - Architecture pattern description
...
**Changes Made**:
- Replaced `"projectName": "MeatyPrompts"` with `"projectName": "MeatyGifts"`
...
**Variable Categories Used**:
- `MeatyGifts` - Project name in metadata
...
- `React with Next.js` - Frontend framework name (e.g., "Next.js")
- `FastAPI` - Backend framework name (e.g., "FastAPI")
- `PostgreSQL with SQLAlchemy ORM` - Database and ORM (e.g., "PostgreSQL with SQLAlchemy")
- `Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates)` - Architecture description
- `1. Database Layer (PostgreSQL schema, migrations)` - Layer breakdown
...
- Router → Service → Repository → DB → `Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates)`
- Layer descriptions → Based on `1. Database Layer (PostgreSQL schema, migrations)`
...
2. **Replace template variables** in both files:
   - Use find & replace: `MeatyGifts` → "YourProject"
