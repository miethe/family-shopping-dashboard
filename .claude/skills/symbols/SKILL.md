...
<!-- Template Variables:
- MeatyGifts - Project or organization name
...
- 1. Database Layer (PostgreSQL schema, migrations) - Detailed architectural layers breakdown
- See docs/project_plans/ for PRDs and planning documentation - Real code examples from the project
-->
...
- File path patterns determine layer based on Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates)
...
Creates layer-specific files based on Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates):
...
- Scripts read `symbols.config.json` to understand MeatyGifts project structure
...
# Result: 5 layer-specific files + original file (based on Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates))
...
Symbols are stored in domain-specific JSON files in the `{{PROJECT_PATHS.symbolsDir}}/` directory. All symbols include a `layer` field for architectural filtering based on Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates).
...
- MeatyGifts specific domains are configured in your `symbols.config.json`

**Backend Layer Files** (when using layer-based split for token efficiency):
- Layer organization matches Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates)
...
**Note**: Actual file names, symbol counts, and organization depend on your MeatyGifts configuration in `symbols.config.json`. See { for your project's specific symbol file locations.
...
```json
{
  ...
  "domain": "frontend",
  "layer": "component",
  "summary": "Reusable component with variants (example from MeatyGifts)"
}
```
...
**Layer tags** (all symbols include one, based on Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates)):
...
- Test layer: `test`
- Your project's specific layers are defined in `symbols.config.json` and match 1. Database Layer (PostgreSQL schema, migrations)
...
- Layered architecture mapping based on Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates)
...
Read this reference to understand how symbols map to MeatyGifts's architectural patterns defined in 1. Database Layer (PostgreSQL schema, migrations).
...
Symbols understand and validate MeatyGifts's layered architecture as defined in Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates).

**Architecture layers are configured per project** based on 1. Database Layer (PostgreSQL schema, migrations):
