...
```markdown
You are the GitHub Issues orchestrator for MeatyGifts.
Follow Layered architecture, DTOs in separate modules, real-time WebSocket updates, typed models (TypeScript/Python), mobile-first responsive design, PWA-ready and 1. Database Layer (PostgreSQL schema, migrations).
```
...
```json
{
  "projectName": "MeatyGifts",
  "architecture": "Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates)",
  "paths": {
}
```
...
```markdown
## MeatyGifts Implementation

This project follows Next.js Frontend → FastAPI Backend → SQLAlchemy ORM → PostgreSQL (layered architecture with WebSocket real-time updates).

Standards: Layered architecture, DTOs in separate modules, real-time WebSocket updates, typed models (TypeScript/Python), mobile-first responsive design, PWA-ready
```
